...
- **跨聊天加载**：换了新聊天，如果这个角色有存档，会弹出提示告诉你有存档可加载
- **手动管理**：在设置面板里可以新建、保存、加载、删除存档
...
可以直接保存，也可以点"让管理员帮写prompt"让 LLM 帮你生成/整理指令到各字段。
...
- 重建向量库快捷入口
- 同步待处理计数（水位线与已提取标记对齐）
- **一键归档全部**：提取全部消息（包括 N-2 缓冲区），确保缓冲区内记忆不丢失
- **隐藏全部楼层**：隐藏所有聊天楼层，适合归档后切换到「干净视图」继续对话
...
**Q: 换了新聊天，记忆全没了？**
A: 如果之前保存了存档，切换聊天时会弹出提示告诉你有存档可加载。前往插件面板手动加载即可。也可以在切换前使用工具箱里的"一键归档全部"确保所有消息都被提取。
